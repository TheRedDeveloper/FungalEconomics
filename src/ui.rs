...
fn resource_label(ui: &mut Ui, label: &str, value: f32, is_missing: bool, color: u32) {
    ...
    ui.element().width(fit!()).height(fit!())
        .layout(|l| l.gap(2))
        .children(|ui| {
            ui.text(&format!("{}:", label), |t| t.font_size(14).color(0xAAAAAA));
...
fn render_tile_button(ui: &mut Ui, state: &mut GameState, base: BaseTileType, assets: &Assets) {
    ...
    ui.element().width(fixed!(140.0)).height(fixed!(180.0))
        ...
        .children(|ui| {
            ...
            ui.text(tile.label(), |t| t.font_size(14).color(0xFFFFFF).alignment(CenterX));
            ...
            ui.element().width(grow!()).height(fit!())
                ...
                .children(|ui| {
                    if let Some(c) = trade.consumes_per_tick {
                        ui.text(&format_resources_short(&c), |t| t.font_size(10).color(0xAAAAAA));
                        ui.text("↓", |t| t.font_size(10).color(0xAAAAAA));
                    }
            ...
            ui.element().width(grow!()).height(fit!())
                ...
                .children(|ui| {
                    ...
                    ui.text(&format!("-{} H2O", cost_w as i32), |t| {
    ...
    ui.element().width(grow!()).height(fit!())
        ...
        .children(|ui| {
            ...
            let out_id = ui.element().width(grow!()).height(fixed!(50.0))
                ...
                .corner_radius(4.0)
                .children(|ui| {
                    ui.text("OUTSTACKED", |t| t.color(0xFFFFFF).alignment(CenterX));
                });
            ...
            let spore_id = ui.element().width(grow!()).height(fixed!(50.0))
                ...
                .corner_radius(4.0)
                .children(|ui| {
                    ui.text("BUY SPORE", |t| t.color(if can_afford { 0x000000 } else { 0x555555 }).alignment(CenterX));
                });
